...
function discovery.handle_discovery(driver, _should_continue)
  log.info("Starting the mySlidingGlassDoorLock Driver Discovery")

  local metadata = {
    type = "LAN",
    -- the DNI must be unique across your hub, using static ID here so that we
    -- only ever have a single instance of this "device"
    device_network_id = "mySlidingGlassDoorLockNetworkId",
    label = "My Sliding Glass Door Lock",
    profile = "my-sliding-glass-door-lock",
    manufacturer = "Dan Jarvis",
    model = "v1",
